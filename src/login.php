...
function userLoginMenu() {
  ...
  $menu = array(
    "login"   => "login.php?page=login",
    "status"   => "login.php",
    "logout"   => "login.php?page=logout",  
  );
...
function userLoginForm($output=null, $outputClass=null) {
  ...
  if(isset($output)) {
    $output = "<p class='right $outputClass' style='display:inline;'><output>$output</output></p>";
  }
  ...
  if(userisAuthenticated()) {
    $disabled = "disabled";
    $disabledInfo = "<em class='quiet small'>Du är inloggad, du måste <a href='?page=logout'>logga ut</a> innan du kan logga in.</em>";
  }
...
function userLogin() {
  ...
  if(isset($_POST['doLogin'])) {
    ...
    if($userAccount == $_POST['account'] && $userPassword == userPassword($_POST['password'])) {
      ...
      $output = "Du lyckades ej logga in. Felaktigt konto eller lösenord.<br><img src='images/YouDidntSayTheMagicWord.gif'>";
      $outputClass = "error";
    }
  }
